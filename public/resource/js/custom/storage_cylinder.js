...
$('#addStorageBtn').click(function () {
    $('#addStorageModal').modal('toggle');
    $("#addStorageForm").trigger("reset");
    $('#id').val('');
    $('.error').text('');
...
$('#addStorageForm').on('submit', function (e) {
    ...
    var returnVal = $("#addStorageForm").valid();
    var formdata = new FormData(this);
    console.log('formdata: ',formdata);
    formdata.append("transaction_use", 'vehicle');
    if (returnVal) {
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 200) {
                    ...
                    setTimeout(
                        $(location).attr('href',ebase_url+'transaction/storage_cylinder'),
                         8000
                         )
...
function setCreateCylinder(list){
    ...
    $('#storageCylTable').dataTable().fnDestroy();
    $('#storageCylList').empty();
    var tblData = '', index=1;
    ...
    $('#storageCylList').html(tblData);
    $('#storageCylTable').DataTable();
