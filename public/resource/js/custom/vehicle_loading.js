...
// addvehicleLoad Button
$('#addvehicleLoadBtn').click(function () {
    $('#addVehicleLoadModal').modal('toggle');
    $("#addVehicleLoadForm").trigger("reset");
    $('#id').val('');
    $('.error').text('');
...
$('#addVehicleLoadForm').on('submit', function (e) {

    e.preventDefault();

    var returnVal = $("#addVehicleLoadForm").valid();
    var formdata = new FormData(this);
    console.log('formdata: ',formdata);
    formdata.append("transaction_use", 'unload');
    if (returnVal) {
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 200) {
                    ...
                    setTimeout(
                        $(location).attr('href',ebase_url+'transaction/vehicle_loading'),
                         8000
                         )
...
function setCreateCylinder(list){
    ...
    $('#vehicleLoadTable').dataTable().fnDestroy();
    $('#vehicleLoadList').empty();
    var tblData = '', index=1;
    ...
    $('#vehicleLoadList').html(tblData);
    $('#vehicleLoadTable').DataTable();
