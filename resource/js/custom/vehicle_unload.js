...
// addUnload Button
$('#addUnloadBtn').click(function () {
    $('#addVehicleUnloadModal').modal('toggle');
    $("#addVehicleUnloadForm").trigger("reset");
    $('#id').val('');
    $('.error').text('');
...
$('#addVehicleUnloadForm').on('submit', function (e) {

    e.preventDefault();

    var returnVal = $("#addVehicleUnloadForm").valid();
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 200) {
                    ...
                    setTimeout(
                        $(location).attr('href',ebase_url+'transaction/vehicle_unload'),
                         8000
                         )
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == 200) {
                ...
                // If there is no data or an error occurred
                var tableBody = $('#vehicleUnloadList');
...
function setCreateCylinder(list){
    ...
    $('#vehicleUnloadTable').dataTable().fnDestroy();
    $('#vehicleUnloadList').empty();
    var tblData = '', index=1;
    ...
    $('#vehicleUnloadList').html(tblData);
    $('#vehicleUnloadTable').DataTable();
